...
large spikes in memory usage, and generally stabilizes at around the peak memory usage.
Therefore, we only observed the peak memory usage, as it seems representable for the
amount of memory which would be required to run each tool in an IDE scenario.

JProfiler provides a live memory usage overview which it profiles every $2$ seconds on a
