...
tree for a list of all nodes which matches the query. For each node, all the tokens which
it covers (the values in all the leaves in its subtree) are extracted and sent to the next
phase.
...
unique token value is mapped to unique integer values. Figure \ref{fig:fingerprint} shows
how a sample Java program could theoretically be fingerprinted. The fingerprint mapping
starts its ``count'' at 2 to make space for some terminating characters. Each fragment is
...
$\mathrm{suffix}(S, i)$ is L-type if $\ArrayAccess{S}{i} > \ArrayAccess{S}{i + 1}$. Similarly,
$\mathrm{suffix}(S, i)$ is S-type if $\ArrayAccess{S}{i} < \ArrayAccess{S}{i + 1}$. If
$\ArrayAccess{S}{i} = \ArrayAccess{S}{i + 1}$, then $\mathrm{suffix}(S, i)$ is the same value as
...
on the first character. If the first character is equal, we have effectively
...
Figure \ref{eq:suffixtypesrecurrence} shows a recurrence which determines the type of each suffix
