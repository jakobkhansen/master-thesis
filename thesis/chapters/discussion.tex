...
that CCDetect-LSP was able to detect almost all type-2 clones.
...
all type-2 clones with only a few token types added to its configuration, but we believe
that this number would increase closer to $100\%$ with more token types added. We believe
that BigCloneBench considers more token types to be allowed differences than our
...
If type-3 clones were to be considered, we need to consider how useful it is to report
...
\Todo{More here about other suffix array based detection tools}
