...
suffix array. Recall that $LF(i) = rank(i, BWT[i]) + C[BWT[i]]$ where $rank(i, BWT[i])$ is the
rank for the character $BWT[i]$ at position $i$ in the BWT, and $C[BWT[i]]$ counts the number
of lexicographically smaller characters than $BWT[i]$ in the BWT.
