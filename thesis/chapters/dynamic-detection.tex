...
of the previous characters. If $C$ is implemented as an array where the $C[c]$ holds the
number of smaller characters than $c$ in the input, finding the correct character can be
done efficiently with a binary search. When we know which character is located at position
$i$ in the first column, we do $select_c(i - C[c])$ to determine where that occurence of
$c$ is located in the last column (the BWT), which is then returned as the result of the
inverse LF function.
...
fingerprint which corresponds to the start of code clones, which will later be mapped by
back to the original source code again.
...
all the nodes with an LCP value above the token treshold parameter.
