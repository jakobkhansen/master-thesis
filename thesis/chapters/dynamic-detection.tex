...
see that some parts of the BWT is preserved, such as \verb|$AA| and \verb|AN|, but some
changes have naturally occurred. We know that we will have exactly one more cyclic-shift
for the string, which starts with the inserted character \verb|B| and ends with the
character at position $2 - 1 = 1$ which is \verb|A|. We also know that exactly one
cyclic-shift will change its last character, which is the cyclic-shift previously starting
at position $2$, but is now at position $3$ after the insert. This cyclic-shift will now
end with the new inserted character, \verb|B|. These are the only changes to the
cyclic-shifts which happen (which matter for the BWT), but it is also possible that the
cyclic-shifts change their ordering because of the new character. For example
\verb|ANANA$B| $<$ \verb|ANA$BAN|, but \verb|ABNANA$B| $>$ \verb|ANA$BABN|. A useful
observation is that only cyclic-shifts of order $j \leq 2$ will have its lexicographical
ordering changed. This is because only in these cyclic-shifts the inserted \verb|B| comes
before the \verb|$|. Since the \verb|$| is the smallest lexicographical character, and no
cyclic-shift will have \verb|$| in the same location, we know that the lexicographical
comparison of two cyclic-shifts will never go past the \verb|$|. Therefore, only the
cyclic-shifts where the inserted character comes before the \verb|$|, has the possibility
of being moved.
...
In table \ref{table:bwtupdate}, we see that the new cyclic-shift is
...
the cyclic-shifts which have changed their lexicographical ordering. We know that only
cyclic-shifts of order $j \leq 2$ can have its ordering changed, and we have already
inserted the cyclic shift of order $2$ at the correct position (the new cyclic-shift).
Therefore, we can again use the LF-function which will allow us to move from the
cyclic-shift of order $2$ to the position where the cyclic-shift of order $1$ should be
located. Initially, before any changes were done to the BWT, we also stored the position
of the cyclic-shift of order $1$, which we found by using the LF-function. With the
knowledge of where the cyclic-shift of order $1$ is, and where it should be, we can move
the cyclic-shift to that position. In this example, we have that the position of the
cyclic-shift of order $1$ was located at position $3$ originally, and doing $LF(5)$ after
the insertion gives us the expected location of the cyclic-shift of order $1$, which is at
position $2$. Therefore, we remove the cyclic-shift at position $3$, and insert it again
at position $2$.
