...
shift, not the whole cyclic-shift. For the string \verb|BANANA$|, we insert a \verb|B| at
position $2$, giving us the string \verb|BABNANA$|. In table \ref{table:bwtupdate} we can
see that some substrings of the BWT is preserved, such as \verb|$AA| and \verb|AN|, but
some changes have naturally occurred. We know that we will have exactly one more $CS$ for
the string, which starts with the inserted character at position $2$, which is \verb|B|.
This $CS$ ends with the character before the inserted character, which is the \verb|A| at
position $1$. We also know that exactly one $CS$ will change its last character, which is
the cyclic-shift previously starting at position $2$, but is now at position $3$ after the
insert. This cyclic-shift will now end with the new inserted character, \verb|B|. These
are the only changes that happen to the BWT characters.
...
In table \ref{table:bwtupdate}, we see that the new $CS$ is \verb|BNANA$BA|, and the
changed $CS$ is \verb|NANA$BAB|. We can find where these cyclic-shifts are located
by first looking up the insert position in the ISA, $\ISA{2} = 6$. This is the
cyclic-shift (\verb|NANA$BA|) where the final character is updated to the inserted
character \verb|B|. To find the position where the new cyclic-shift should be inserted, we
can use the LF-function to move from the current cyclic-shift to the previous. Computing
$LF(6)$ after the substitute gives us $5$, which is where the new cyclic-shift should be
inserted.
...
observation is that only $CS_j$ where $j \leq 2$ will have their lexicographical ordering
...
We know that only $CS_j$ where $j \leq 2$ can have their ordering changed, and we have
already inserted $CS_2$ at the correct position (the new cyclic-shift). Therefore, we can
again use the LF-function which will allow us to move from the cyclic-shift of order $2$
to the position where the cyclic-shift of order $1$ should be located. Initially, before
any changes were done to the BWT, we also stored the position of the cyclic-shift of order
$1$, which we found by using the LF-function. With the knowledge of where the cyclic-shift
of order $1$ is, and where it should be, we can move the cyclic-shift to that position. In
this example, we have that the position of the cyclic-shift of order $1$ was located at
position $3$ originally, and doing $LF(5)$ after the insertion gives us the expected
location of the cyclic-shift of order $1$, which is at position $2$. Therefore, we remove
the cyclic-shift at position $3$, and insert it again at position $2$.

Before performing the row-move, we also compute $LF(3) = 4$ which will give us the
position of the cyclic-shift of order $0$. From there we can continue to compare the
actual position and expected position of the order $0$ cyclic-shift by computing $LF(2) =
4$ after the row-move, which is the expected position of the cyclic-shift of order
$0$. Since the expected position and actual position is the same, we are done, and we know
that every position of the BWT is now in the correct location.

\subsubsection{Updating SA}
The suffix array is updated similarly. When we insert the new cyclic-shift at position $5$
in the BWT, we similarly insert the value $2$ at position $5$ in SA. We insert the value
$2$ because the new suffix which corresponds to the new cyclic-shift starts at the
position where the new character was inserted, which was $2$. When the new cyclic-shift is
inserted, we are now in an invalid state for the suffix array, because we have two
elements with the value $2$. Note that a suffix array is always a permutation of the
values $(0..n)$ where $n$ is the number of characters in the input. Since we have inserted
a new suffix in the suffix array which is the 2nd smallest suffix, all the previous
suffixes which had a value $j \geq 2$ is now incremented. After incrementing all these
suffixes, we are now in a valid suffix array state, but similarly to our BWT, some
...
\begin{algorithm}[t]
  \SetAlgoLined\DontPrintSemicolon
    \algo{\UpdateSuffixArrayInsert{SA, ISA, BWT, pos, ch}}{
        ...
        $\Var{pointOfInsertion} \gets \LF{$\Var{BWT}, \Var{pos}$}$ \;
        \If{$\Var{storedLetter} < \Var{ch}$} {
            $\Var{pointOfInsertion} \gets \Var{pointOfInsertion} + 1$
        } \;

        \Comment{Insert storedLetter in BWT at pointOfInsertion}
        \Insert{$\Var{BWT}, \Var{pointOfInsertion}, \Var{storedLetter}$}\;\;
        \Comment{Insert position in SA at pointOfInsertion}
        \Insert{$\Var{SA}, \Var{pointOfInsertion}, \Var{position}$}\;\;

        \Comment{Increment all values in SA greater than position}
        \IncrementGreaterThan{$\Var{SA}, \Var{position}$}\;\;


        \If{$\Var{pointOfInsertion} \leq \Var{previousCS}$} {
            $\Var{previousCS} \gets \Var{previousCS} + 1$
        } \;


        $\Var{pos} \gets \Var{previousCS}$ \;
        $\Var{expectedPos} \gets \LF{$\Var{pointOfInsertion}$}$ 

        \While{$\Var{pos} \neq \Var{expectedPos}$}{ \;

            $\Var{newPos} \gets \LF{$\Var{pos}$}$ \;\;

            \Comment{Delete value at pos and reinsert at expectedPos in BWT and SA}
            \MoveRow{$\Var{pos}, \Var{expectedPos}$} \;\;

            $\Var{pos} \gets \Var{newPos}$ \;
            $\Var{expectedPos} \gets \LF{$\Var{expectedPos}$}$ \;
...
string instead of a single character by adding a loop which changes the
\verb|pointOfInsertion| to the previous $CS$ with the LF-function, and inserts all the
characters into the BWT and SA backwards. This is more efficient than calling the single
character algorithm multiple times, because the reordering stage is only performed once. 
