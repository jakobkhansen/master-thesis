...
Incremental clone detection avoids recomputation of already computed results when
performing code clone detection on consecutive revisions. A revision of a code base is one
version of a code base which has been changed from a previous revision, and when edited
becomes the next revision. A revision of the code base could for example be a certain Git commit
of the code base, or one could say a new revision of the code base is created for each
edit performed while programming. These two scenarios have respectively been called the
evolution scenario and the IDE scenario. Since in most cases, an entire codebase will not change
entirely between revisions, many code clones will remain between revisions, and therefore
a lot of processing can be avoided. However, this is not a simple problem, since changes
in a single location can affect clone detection results across the entire codebase.
...
\begin{itemize}
    ...
    \item Another tool, SHINOBI, which is a plugin for the Visual Studio editor, can
        ...
        one can assume that the suffix array is recomputed from scratch in each search.~\cite{SHINOBI}.
...
code-actions and commands, which the LSP server communicates to the client in order to
perform custom actions defined by the server.
...
nodes or subtrees. Traversing AST's will be useful in our clone detection algorithm in the
following chapters.

In our detection algorithm we will need to be able to parse our source code into a CST
...
much. Therefore, when source code is edited it would be more efficient to use an
incremental parsing algorithm to parse the source code when only a small region has been
changed. Incremental parsing is the process of parsing only parts of a syntax tree
whenever an edit is performed in a file, and reusing subtrees of the previous syntax tree
which did not change. The motivation behind incremental parsing is to have a readily
...
subtree in the AST could save time compared to parsing the entire AST from scratch. In
larger programs we can see significantly more reusage of subtrees, since an edit to a few
lines would likely require few nodes to be parsed compared to the number of nodes reused.
