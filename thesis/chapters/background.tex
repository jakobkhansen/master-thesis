...
Duplicated code is often called code clones in the literature. A pair of code snippets which are
...
There are multiple existing clone detection tools which integrate into IDEs. The IDE-based
...
contains a lexicographical sorting of all suffixes in $S$. The suffix array does not
contain the actual suffixes, but it contains integers pointing to the index where the
suffix starts in $S$. The suffix array is therefore a permutation of the values in the
range $0$ to $N-1$. Conversely, the inverse suffix array (ISA) contains integers
describing which rank the suffix at a given position has. The rank of a suffix is its
lexicographical ordering in $S$. ISA is therefore the inverse array of SA, such that if
$\text{SA}[i] = n$, then $\text{ISA}[n] = i$.
...
\begin{definition}[LCP array] Let S be a text of length N and SA be the suffix array of S.
...
Algorithm \ref{alg:bwt} shows how the BWT of S can be computed directly from the SA of S
...
$S$ in sorted order to determine the previous CS with only the BWT. We can compute the
...
CS. This function is called the Last-to-first function (LF function). Computing the result
of the LF function can be done efficiently by using a rank/select data structure such as a
wavelet tree~\cite{WaveletTree} or wavelet matrix~\cite{WaveletMatrix} to compute the rank
of all the characters in the BWT, and an array $C$ which contains the number of
occurrences of each letter in the BWT. In table \ref{table:bwt}, we can compute the
original $S$ from the BWT by starting at position $4$, because that is the position of the
\$, then use the LF-function, $LF(4) = 0$, which means the second to last character is
$BWT[0] = A$. $LF(0) = 1$, which means the next character is $BWT[1] = N$, and so on.

The LF function will also be useful in the detection algorithm, when we dynamically update
the suffix array of $S$, given the changes to the BWT.
