...
(SACA). All the fingerprints which are stored in each document object are concatenated to
be stored in a single integer array (with terminators after each fragment), which the
suffix array (SA), inverse suffix array (ISA) and longest-common prefix array (LCP) is
computed from.

We have utilized a straight-forward implementation of the ``Induced sorting
variable-length LMS-substrings'' algorithm which calculates a suffix array in linear time. The following section will give an overview of
how the algorithm works, but readers interested in the full details are referred
elsewhere\cite{LinearTimeSuffixArraySAIS}. 
...
\begin{definition}[LMS-characters]
