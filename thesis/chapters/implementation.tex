  ...
  \SetAlgoLined\DontPrintSemicolon
    \algo{\SimpleCloneExtraction{S, ISA, LCP}}{
        ...
        \For{$i \in 0..n - 1$}{
            ...
            \If{$\text{LCP}[\text{ISA}[i]] \geq \text{THRESHOLD}$}{
                $\text{clones.add}(i)$
            }
        }
    ...
    \algo{\CloneExtraction{S, ISA, LCP}}{
        ...
        \For{$i \in 0..n - 1$}{
            ...
            \If{$\text{LCP}[\text{ISA}[i]] \geq \text{THRESHOLD}$}{
                ...
                \While{$i < n \text{ and } LCP[ISA[i]] < LCP[ISA[i - 1]]$}{
                    $i \gets i + 1$
                }
...
\begin{algorithm}[htp!]
  \SetAlgoLined\DontPrintSemicolon
    \algo{\SourceMap{documents, i}}{
        $left \gets 0$ \;
        $right \gets \text{documents.length - 1}$ \;\;


        \While{$left \leq right$}{
            $mid \gets (left + right) / 2$

            \uIf{$documents[mid].fingerprintEnd < i$}{
                $left = mid + 1$
            }
            \uElseIf{$documents[mid].fingerprintStart > i$}{
                $right = mid - 1$
            }
            \Else {
                $document \gets documents[mid]$ \;
                $range \gets document.ranges[i - document.fingerprintEnd]$

                \Return (document.uri, range)
            }
