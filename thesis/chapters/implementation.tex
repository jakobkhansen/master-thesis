...
The tool is integrated into IDEs via the Language server protocol. The tool starts up as
...
clones are displayed in the file as an ``Information'' diagnostic, and when hovered over,
the client shows the \verb|DiagnosticRelatedInformation| part of the diagnostic where all
the matching clones are displayed and can be clicked to navigate to the corresponding
match. For a client which may not implement the \verb|DiagnosticRelatedInformation|
message (which was added in version 3.16), invoking the code-action to navigate to the
matching clone is another option.
...
\begin{algorithm}[t]
  \SetAlgoLined\DontPrintSemicolon
    \algo{\GetCloneMap{index, cloneIndices}}{
        ...
        $\Var{cloneMap} \gets \textrm{Map\textless int, CodeClone\textgreater}$ \;

        
        \For{$\Var{i} \From 0 \To \Var{n} - 1$}{
            $\Var{firstIndex} \gets \ArrayAccess{\Var{cloneIndices}}{\Var{i}}$ \;
            $\Var{secondIndex} \gets \ArrayAccess{\Var{cloneIndices}}{\Var{i}}$ \;
            $\Var{cloneSize} \gets \LCP{\ISA{\Var{firstIndex}}} - 1$ \;\;


            \Comment{Get sources for the first and last token of both clones}
            $\Var{firstSourceStart} \gets \SourceMap{$\Access{\Var{index}}{\Var{documents}},
            \Var{firstIndex}$}$ \;
            $\Var{firstSourceEnd} \gets \SourceMap{$\Access{\Var{index}}{\Var{documents}},
            \Var{firstIndex + cloneSize}$}$ \;\;

            $\Var{secondSourceStart} \gets \SourceMap{$\Access{\Var{index}}{\Var{documents}},
            \Var{secondIndex}$}$ \;
            $\Var{secondSourceEnd} \gets \SourceMap{$\Access{\Var{index}}{\Var{documents}},
            \Var{secondIndex + cloneSize}$}$ \;\;

            \Comment{Build range between both source-pairs for both clones}
            $\Var{firstRange} \gets (\Access{\Var{firstSourceStart}}{\Var{start}},
            \Access{\Var{firstSourceEnd}}{\Var{end}})$ \;
            $\Var{secondRange} \gets (\Access{\Var{secondSourceStart}}{\Var{start}},
            \Access{\Var{secondSourceEnd}}{\Var{end}})$ \;\;

            \Comment{Get the document of each clone}
            $\Var{firstDocument} \gets \Access{\Var{firstSourceStart}}{\Var{document}}$ \;
            $\Var{secondDocument} \gets \Access{\Var{firstSourceStart}}{\Var{document}}$ \;\;
