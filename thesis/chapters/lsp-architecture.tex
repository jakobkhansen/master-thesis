...
focuses on our novel algorithm based on dynamic suffix arrays. The current chapter will
look at the LSP integration and how source code is initially indexed, while the two next
chapters will discuss first how clones are initially detected by CCDetect-LSP and then how
this analysis in incrementally updated, faster than running the initial detection again.

CCDetect-LSP is integrated into IDEs via the Language server protocol. The tool starts up
as an LSP server, which an IDE client can connect to and send/receive messages from. The
goal of the LSP client-server interaction is to give users of the tool an overview of all
clones as they appear in source code, and allow them to navigate between matching clones.
The LSP server is implemented in Java with the LSP4J library which provides an abstraction
layer on top of the protocol which is easier to work with programmatically.
...
\verb|DiagnosticRelatedInformation| message\footnote{This feature was added to LSP version
3.16, released in 2020}, invoking the code-action to navigate to the matching clone is
another option.
...
\begin{itemize} 
    ...
    \item A programmer wants to jump to one of the matches of a code clone in their IDE.
      ...
      \item A programmer wants to remove a set of clones by applying the
          ...
          performs the necessary refactorings, and will see that all the previously
          detected clones are gone when the LSP server updates its analysis.
